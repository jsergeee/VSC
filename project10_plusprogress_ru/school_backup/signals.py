...
from django.apps import apps  # ВАЖНО: импортируем apps
...
# НЕ ИСПОЛЬЗУЙТЕ get_user_model() на уровне модуля!
# User = get_user_model()  # <- УДАЛИТЕ ЭТУ СТРОКУ

@receiver(post_save, sender='auth.User')  # Используйте строку вместо модели!
def create_auth_token(sender, instance=None, created=False, **kwargs):
    """Автоматически создаем токен при создании любого пользователя"""
    if created:
        # Импортируем Token внутри функции
        from rest_framework.authtoken.models import Token
        token, created = Token.objects.get_or_create(user=instance)
        logger.info(f"✅ Токен создан для {instance.username} ({instance.role})")


@receiver(m2m_changed, sender='school.Lesson.students.through')  # Используем строку!
...
@receiver(post_save, sender='school.LessonAttendance')  # Используем строку!
def attendance_created(sender, instance, created, **kwargs):
    ...
    if created:
        ...
        # Получаем модели через apps.get_model() или импортируем внутри функции
        Notification = apps.get_model('school', 'Notification')
        ...
        LessonAttendance = apps.get_model('school', 'LessonAttendance')
...
@receiver(post_save, sender='school.LessonReport')  # Используем строку!
...
@receiver(post_save, sender='school.Payment')  # Используем строку!
...
@receiver(post_save, sender='auth.User')  # Используем строку для User
def send_welcome_notification(sender, instance, created, **kwargs):
    """Приветственное уведомление для новых пользователей"""
    if created:
        # Получаем модель внутри функции
        Notification = apps.get_model('school', 'Notification')
...
@receiver(post_delete, sender='school.Payment')  # Используем строку!
